...
export async function deployDiamond(options: {
	...
	const { ethers, gwei } = options
	const wei = gwei ? gwei * 1000000000 : undefined
	...
	const diamond = await Diamond.deploy({ gasPrice: wei })
	...
	for (const facetName of facetNames) {
		...
		const facet = await Facet.deploy({ gasPrice: wei })
